...
class Find_Stock(Frame):
    ...
    def __init__(self,master):
        ...
        self.Check_Box_Inputs= [("promotor Shareholdor (%)\t\t",19,"Share_Holdor_cb"),
                                ("promotor pledge Shareholdor (%)\t\t\t",20,"Pledge_Holdor_cb"),
                                ("Equality Capital limit (cr)\t\t",3,"Capital_cb"),
                                ("Book Value (cr)\t\t ",6,"Profit_cb"),
                                ("Sale Growth (%)\t\t",5,"Sale_cb")]
    ...
    def __Create_Window_Design(self):
        ...
        for Index,Value in enumerate(self.Check_Box_Inputs):
            self.List_Of_Criteria_Checkbox.append(Checkbutton(self,text=Value[0],\
                                                  onvalue=Value[1],variable=Value[2], offvalue=0).\
